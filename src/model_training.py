...
class ModelTraining:
    ...
    # Function to Train Model (LogisticRegression) using loaded data
    def train_model(self):
        try:
            ...
            logger.info("Model trained and saved succesfully in artefacts Folder...")

        except Exception as e:
            logger.error(f"Error while training model {e}")
            raise CustomException("Failed to train model" , e)
    
    # Function to evaluated trained model performance on test set
    def evaluate_model(self):
        try:

            y_pred = self.clf.predict(self.X_test)

            # performance evaluation using 4 metrics below
    ...
    # Function when envoked runs all the required above functions sequentially
    def run(self):
        self.load_data()
        self.train_model()
        self.evaluate_model()
...
# For Model Training Testing Purposes
